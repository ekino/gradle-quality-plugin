...
openssl aes-256-cbc -K $encrypted_5ecdf0ba2ee0_key -iv $encrypted_5ecdf0ba2ee0_iv -in "${ENCODED_SIGN_FILE_PATH}" -out "${SIGN_FILE_PATH}" -d

if [[ "${PROJECT_VERSION}" =~ .*-SNAPSHOT$ ]]; then
    ./gradlew check jacocoTestReport sonarqube publish -PossrhUrl="${PUBLISH_SNAPSHOT_REPO_URL}" -PossrhUsername="${PUBLISH_REPO_USERNAME}" -PossrhPassword="${PUBLISH_REPO_PASSWORD}" -Psigning.keyId="${GPG_KEY_ID}" -Psigning.password="${GPG_PASSPHRASE}" -Psigning.secretKeyRingFile="${SIGN_FILE_PATH}"
